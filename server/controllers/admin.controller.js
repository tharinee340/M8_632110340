...
module.exports =  {
    ...
    add: async (req, res, next) => {
        try {
            let books = new book(req.body) //ใส่ข้อมูลเข้าไป
    ...
    update: async (req, res, next) => {
        try {
            const { id } = req.params
            let book_update = await book.findByIdAndUpdate(id , req.body, {new: true});
    ...
    deleteBook: async (req, res, next) => {
        try {
            ...
            const data = { message: "Data has been Deleted", book_delete}
            res.status(200).json(data)
        } catch(err) {
